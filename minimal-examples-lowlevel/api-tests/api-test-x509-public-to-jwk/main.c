...
print_jwk_info(struct lws_jwk *jwk)
{
	...
	for (i = 0; i < LWS_GENCRYPTO_MAX_KEYEL_COUNT; i++) {
		if (jwk->e[i].buf && jwk->e[i].len > 0) {
			...
			hex[hex_len * 2] = '\0';
			lwsl_user("  Element[%d]: %u bytes - %s%s", i, jwk->e[i].len,
				 hex, jwk->e[i].len > 8 ? "..." : "");
		}
	}
}
...
run_test_case(const struct test_case *tc, const char *cert_dir)
{
	...
	lws_snprintf(cert_full_path, sizeof(cert_full_path), "%s/%s",
		     cert_dir, tc->cert_path);
	...
	if (ret == tc->expected_result) {
		if (ret == 0) {
			if (jwk.kty == (int)tc->expected_kty) {
				...
				if (jwk.kty == LWS_GENCRYPTO_KTY_RSA) {
					...
					if (!nlen || !elen) {
						lwsl_user("FAILED: RSA n/e length invalid (n=%u, e=%u)",
							 nlen, elen);
						result = -1;
					} else if (tc->rsa_min_bits > 0 &&
						   nlen < (unsigned int)(tc->rsa_min_bits / 8)) {
						lwsl_user("FAILED: RSA modulus too short: %u bytes (< %d bytes)",
				...
				if (result == 0 && jwk.kty == LWS_GENCRYPTO_KTY_EC) {
					...
					if (!xlen || !ylen) {
						lwsl_user("FAILED: EC x/y length invalid (x=%u, y=%u)",
							 xlen, ylen);
						result = -1;
					} else {
				...
				lwsl_user("FAILED: Expected key type %d, got %d",
		...
		lwsl_user("FAILED: Expected return %d, got %d",
